...
class BrowserSwitcherCore {
 public:
  ...
  bool InvokeAlternativeBrowser(const std::wstring& url);
  // Invokes Chrome and loads |url| in it.
  bool InvokeChrome(const std::wstring& url);
  ...
  const std::wstring& GetAlternativeBrowserPath();
  ...
  const std::wstring& GetAlternativeBrowserParameters();
  ...
  const std::wstring& GetChromePath();
 ...
 private:
  ...
  // Retrieves the configuration file path based on %LOCALAPPDATA%.
  ...
  void ProcessUrlList(const UrlList& list, UrlListTypes* types);
  ...
  std::wstring GetBrowserLocation(const wchar_t* key_name);

  // Retrieves a string value from the registry stored at |key| or an empty
  // string if missing.
  std::wstring ReadDefaultRegValue(HKEY key);
